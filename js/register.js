...
function getPhoneUpdateRequest(){
	...
	if ((xmlHttp.readyState == 0)||  (xmlHttp.readyState == 4)){
		...
		xmlHttp.open("post", "checkPhone.php", true);
		xmlHttp.onreadystatechange=phoneUpdateCallback;
		xmlHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlHttp.send("phone="+phone);
	}
	...
	if (xmlHttp.readyState == 4){
		if (xmlHttp.status == 200){
			...
			if (code=="1" || code=="2"){
				...
				document.getElementById("phoneMessage").innerHTML = '<span style="color:#50c878"><i class="glyphicon glyphicon-ok"></i></span>';


			}else{
				document.getElementById("submitBtn").disabled = true;
				document.getElementById("phoneMessage").innerHTML =  '<span style="color:#8b0000 "><i class="glyphicon glyphicon-remove"></i></span>';
			} 
